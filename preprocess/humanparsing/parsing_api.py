...
def delete_irregular(logits_result):
  ...
  contours_dress, hierarchy_dress = cv2.findContours(dresses.astype(np.uint8), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_TC89_L1)
  area_dress = []
...
def refine_hole(parsing_result_filled, parsing_result, arm_mask):
  filled_hole = (
    cv2.bitwise_and(np.where(parsing_result_filled == 4, 255, 0), np.where(parsing_result != 4, 255, 0))
    - arm_mask * 255
  )
...
def onnx_inference(session, lip_session, input_dir):
  ...
  with torch.no_grad():
    for _, batch in enumerate(tqdm(dataloader)):
      ...
      refine_hole_mask = refine_hole(
        parsing_result_filled.astype(np.uint8), parsing_result.astype(np.uint8), arm_mask.astype(np.uint8)
      )
  ...
  neck_mask = np.logical_and(
    np.logical_not((parsing_result_lip == 13).astype(np.float32)), (parsing_result == 11).astype(np.float32)
  )
