...
def get_arguments():
  ...
  parser.add_argument("--use_val", type=bool, default=False, help="use train+val set for finetuning or not")
  parser.add_argument(
    "--train_img_dir", type=str, default="../data/instance-level_human_parsing/Training/Images", help="train image path"
  )
  parser.add_argument(
    "--train_anno_dir",
    type=str,
  ...
  parser.add_argument(
    "--val_img_dir", type=str, default="../data/instance-level_human_parsing/Validation/Images", help="val image path"
  )
  parser.add_argument(
    "--val_anno_dir",
    type=str,
...
def main(args):
  ...
  for image_name in os.listdir(args.train_img_dir):
    ...
    for i in range(1, len(human_gt_labels)):
      ...
      annotation_info = pycococreatortools.create_annotation_info(
        segmentation_id, image_id, category_info, binary_mask, image.size, tolerance=10
      )
  ...
  coco_output_val = {"info": INFO, "licenses": LICENSES, "categories": CATEGORIES, "images": [], "annotations": []}
  ...
  for image_name in os.listdir(args.val_img_dir):
    image = Image.open(os.path.join(args.val_img_dir, image_name))
    image_info = pycococreatortools.create_image_info(image_id_val, image_name, image.size)
    ...
    for i in range(1, len(human_gt_labels)):
      ...
      binary_mask = np.uint8(human_mask == i)
      annotation_info = pycococreatortools.create_annotation_info(
        segmentation_id_val, image_id_val, category_info, binary_mask, image.size, tolerance=10
      )
      if annotation_info is not None:
