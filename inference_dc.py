...
class DresscodeTestDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    im_pil_big = Image.open(os.path.join(self.dataroot, "images", im_name)).resize((self.width, self.height))
    ...
    pose_img = Image.open(os.path.join(self.dataroot, "image-densepose", im_name))
  ...
  def get_agnostic(self, parse_array, pose_data, category, size):
    ...
    if category == "dresses" or category == "upper_body":
      ...
      if wrist_right[0] <= 1.0 and wrist_right[1] <= 1.0:
        ...
        arms_draw.line(
          [wrist_left, elbow_left, shoulder_left, shoulder_right, elbow_right, wrist_right], "white", 30, "curve"
        )
    ...
    parse_mask += np.logical_or(
      parse_mask,
      np.logical_and(np.array(parse_head, dtype=np.uint16), np.logical_not(np.array(parse_head_2, dtype=np.uint16))),
    )
...
def main():
  ...
  image_encoder = CLIPVisionModelWithProjection.from_pretrained(
    args.pretrained_model_name_or_path, subfolder="image_encoder", torch_dtype=torch.float16
  )
  UNet_Encoder = UNet2DConditionModel_ref.from_pretrained(
    args.pretrained_model_name_or_path, subfolder="unet_encoder", torch_dtype=torch.float16
  )
  text_encoder_one = CLIPTextModel.from_pretrained(
    args.pretrained_model_name_or_path, subfolder="text_encoder", torch_dtype=torch.float16
  )
  text_encoder_two = CLIPTextModelWithProjection.from_pretrained(
    args.pretrained_model_name_or_path, subfolder="text_encoder_2", torch_dtype=torch.float16
  )
  tokenizer_one = AutoTokenizer.from_pretrained(
    args.pretrained_model_name_or_path, subfolder="tokenizer", revision=None, use_fast=False
  )
  tokenizer_two = AutoTokenizer.from_pretrained(
    args.pretrained_model_name_or_path, subfolder="tokenizer_2", revision=None, use_fast=False
  )
