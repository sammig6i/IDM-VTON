...
class DensePoseCSEBaseSampler(DensePoseBaseSampler):
  ...
  def __init__(self, cfg: CfgNode, use_gt_categories: bool, embedder: torch.nn.Module, count_per_class: int = 8):
    """
    Constructor
  ...
  def _sample(self, instance: Instances, bbox_xywh: IntTupleBox) -> Dict[str, List[Any]]:
    ...
    closest_vertices = squared_euclidean_distance_matrix(selected_embeddings[index_sample], self.embedder(mesh_name))
