  ...
  for dataset_name in dataset_names:
    ...
    for cat_id in cat_ids:
      ...
      merged_categories[cat_id_mapped].append(
        _DatasetCategory(
          id=cat_id, name=cat_name, mapped_id=cat_id_mapped, mapped_name=cat_name_mapped, dataset_name=dataset_name
        )
      )
...
def _warn_if_merged_different_categories(merged_categories: _MergedCategoriesT) -> None:
  ...
  for cat_id in merged_categories:
    ...
    if len(merged_categories_i) > 1 and not all(cat.name == first_cat_name for cat in merged_categories_i[1:]):
      cat_summary_str = ", ".join([f"{cat.id} ({cat.name}) from {cat.dataset_name}" for cat in merged_categories_i])
...
def build_data_sampler(cfg: CfgNode, sampler_cfg: CfgNode, embedder: Optional[torch.nn.Module]):
  if sampler_cfg.TYPE == "densepose_uniform":
    ...
    data_sampler.register_sampler(
      "pred_densepose", "gt_densepose", DensePoseUniformSampler(count_per_class=sampler_cfg.COUNT_PER_CLASS)
    )
    ...
    data_sampler.register_sampler(
      ...
      "gt_densepose",
      DensePoseConfidenceBasedSampler(
        confidence_channel="sigma_2", count_per_class=sampler_cfg.COUNT_PER_CLASS, search_proportion=0.5
      ),
    )
    data_sampler.register_sampler("pred_densepose", "gt_masks", MaskFromDensePoseSampler())
    return data_sampler
    ...
    data_sampler.register_sampler(
      ...
      "gt_densepose",
      DensePoseConfidenceBasedSampler(
        confidence_channel="fine_segm_confidence", count_per_class=sampler_cfg.COUNT_PER_CLASS, search_proportion=0.5
      ),
    )
    data_sampler.register_sampler("pred_densepose", "gt_masks", MaskFromDensePoseSampler())
    return data_sampler
    ...
    data_sampler.register_sampler(
      ...
      "gt_densepose",
      DensePoseConfidenceBasedSampler(
        confidence_channel="coarse_segm_confidence", count_per_class=sampler_cfg.COUNT_PER_CLASS, search_proportion=0.5
      ),
    )
    data_sampler.register_sampler("pred_densepose", "gt_masks", MaskFromDensePoseSampler())
    return data_sampler
...
def build_inference_based_loader(
  cfg: CfgNode, dataset_cfg: CfgNode, model: torch.nn.Module, embedder: Optional[torch.nn.Module] = None
